...
1. 登录工程师或管理员账号，进入“机器视觉”。
2. 点击“新增瓶盖检测”，选择一个样图目录。
3. 确保目录按文件名排序后的第一张图片是合格、有瓶盖样图。
4. 使用 0 到 1 的归一化坐标设置 ROI，使检测区域覆盖瓶盖。
5. 设置匹配阈值，点击“教学并测试当前合格样图”。
6. 教学成功后点击“发布任务并注册报警”。
7. 返回工作台点击“开始检测”，或使用“手动触发一次”进行单帧验证。
8. 检测结果会同步显示在实时图像、统计卡片和当前运行历史中；缺盖图片按任务和日期自动保存。
